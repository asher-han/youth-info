...
> **플랫폼(웹 사이트 도메인) 등록은 하지 않아도 됩니다.**
> 다른 카카오 연동 가이드에는 대부분 들어 있지만, "나에게 보내기"에는 필요 없는
> 단계라 실제로 빼고 검증했습니다. 필요한 URL 등록은 3번의 Redirect URI 하나뿐입니다.

> **앱 키는 URL 등록과 무관합니다.** REST API 키는 앱을 만드는 순간 자동 발급됩니다.
> Redirect URI 는 6번(인가코드 받기)에서만 쓰이므로, URL 등록이 막혀도 키는 이미
> 손에 있습니다.
...
2. **Redirect URI 등록** 클릭 → `http://localhost:3000/oauth` 입력 후 저장
   - **경로(`/oauth`)까지 포함**해야 합니다
   - `http://` 를 반드시 붙입니다. 스킴을 빼면 `유효하지 않은 URL` 이 납니다

> 서버를 띄울 필요는 없습니다. 6번에서 브라우저가 이 주소로 튕겨 나갈 때
...
인가코드를 받기 전에 지금까지의 설정이 맞는지 확인할 수 있습니다.

```bash
python3 scripts/kakao_token.py check
```

`✅ REST API 키·Client Secret 정상` 이 나오면 6번으로 진행합니다.
...
> `KOE010` (client_secret 불일치/누락) 이 뜨면 2번의 Client Secret 이
> `.env` 에 들어갔는지 확인하세요.
