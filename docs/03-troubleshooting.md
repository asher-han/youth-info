...
| 칸 | 올바른 값 |
|---|---|
| 앱 설정 → 플랫폼 → 사이트 도메인 | `http://localhost:3000` (스킴 포함, 경로·끝슬래시 없음) |
| 제품 설정 → 카카오 로그인 → Redirect URI | `http://localhost:3000/oauth` (경로 포함) |

Redirect URI 칸은 **카카오 로그인 활성화를 ON 한 뒤에야** 입력이 먹습니다.
...
`docs/01-kakao-setup.md` 5~6번을 다시 하고 `KAKAO_REFRESH_TOKEN` Secret 을
